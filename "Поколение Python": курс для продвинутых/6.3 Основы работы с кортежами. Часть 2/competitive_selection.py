...
Программа должна вывести сначала все введённые строки с фамилиями и оценками учеников в том же порядке. Затем следует пустая строка, а затем выводятся 
...
n = int(input())
students = tuple([tuple([j for j in input().split()]) for i in range(n)])
for i in range(n):
    print(*students[i])
print()
for i in range(n):
