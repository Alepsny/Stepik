...
Программа должна вывести два числа: номер строки и номер столбца, в которых стоит наибольший элемент таблицы. Если таких элементов несколько, то выводится тот, 
...
for i in range(n):
    new = input().split()
    mult.append(new)
x, y, max_m = 0, 0, int(mult[0][0])
for i in range(n):
    for j in range(m):
        if int(mult[i][j]) > max_m:
            max_m, x, y = int(mult[i][j]), i, j
print(x, y)
