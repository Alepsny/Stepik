...
В первой строке подаётся число n, (0<n<1000) – количество чисел в наборе. В последующих nn строках вводятся целые числа, составляющие набор 
(могут повторяться). Затем следует целое число, которое является или не является произведением двух каких-то чисел из набора.
...
num = int(input())
for i in range(n):
    if s[i] != 0 and num % s[i] == 0:
        for j in range(i + 1, n):
            if s[j] != 0 and num / s[i] / s[j] == 1:
                flag = True
                break
if flag == True or num == 0:
    print('ДА')
else:
    print('НЕТ')
